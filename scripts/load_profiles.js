...
function pickPlaystyle(playstyle) {
	...
	$.each(chardata.characters, function(i, v) {
		if (v.Playstyle.indexOf(playstyle) > -1) {
			v.Points++;
			return;
		}
	});
...
function getResults() {
	...
	var resicon = results2[0].split(",");
	img = ("<img src='char/"+resicon[0]+".png'>");
	other = "";
	runners = "";
	if (results.length > 1) {
	...
	if (runner.length > 0 && runner[0] != "Nobody") {
	...
	if (runner[0] == "Nobody") {
		runners = "<p>Nobody else even came close to being your result!</p>";
	}
